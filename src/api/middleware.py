...
from .auth import AuthError, AuthzError, ServiceTokenValidator

logger = logging.getLogger(__name__)


class AuthMiddleware(BaseHTTPMiddleware):
    worker_paths = ("/api/v2/agents",)
    ...
    async def dispatch(
        ...
        if is_api and not is_token_path:
            ...
            if not bearer.startswith("Bearer ") and not session_token:
                return JSONResponse(
                    status_code=401,
                    content={"detail": "Unauthorized"},
                )
            if self._is_worker_path(request.url.path):
                ...
                try:
                    principal = (
                        ServiceTokenValidator.from_env()
                        .validate(token)
                    )
                except AuthzError:
                    return JSONResponse(
    ...
    def _is_worker_path(self, path: str) -> bool:
        return any(
            ...
            for prefix in self.worker_paths
        )
